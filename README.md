...
Manim Teach 用于把一张题目图片自动转成中文解题讲解视频。当前实现入口是根目录脚本 `solve_image_to_video.py`：它会调用支持图片输入的模型 API 识别题目、生成解题步骤、讲稿、Manim 分镜和 ManimCE 源码，然后在本地 Manim 环境中渲染出视频。

## 当前能力

- 输入单张题目图片。
- 调用 OpenAI 或 OpenAI-compatible API。
...
脚本本身只依赖 Python 标准库；视频渲染依赖本机已有的 ManimCE conda 环境。

## 环境要求

- Python 3.8 或更高版本。
- 可访问的模型 API，默认使用 `https://api.openai.com/v1`。
- API key 环境变量，默认读取 `OPENAI_API_KEY`。
...
| `--model` | `gpt-4.1` 或 `MODEL_NAME` | 模型名称 |
| `--base-url` | `https://api.openai.com/v1` 或 `MODEL_API_BASE_URL` | 模型 API base URL |
| `--api-url` | 无 | 完整 API URL，设置后覆盖 base URL |
| `--api-style` | `chat` 或 `MODEL_API_STYLE` | `chat` 或 `responses` |
| `--api-key-env` | `OPENAI_API_KEY` 或 `MODEL_API_KEY_ENV` | 读取 API key 的环境变量名 |
| `--json-mode` | `schema` | `schema`、`json_object` 或 `none` |
...
其中 `solve_image_to_video.py` 是当前实际执行入口。
